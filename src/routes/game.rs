...
use crate::{routes::game, AppState};
...
pub async fn get_games(data: web::Data<AppState>) -> impl Responder {
    ...
    if query_result.is_err() {
        let message: &str = "error fetching games";
...
async fn create_game(body: web::Json<CreateGameSchema>, data: web::Data<AppState>) -> impl Responder {
    println!("Received raw JSON: {:?}", body);
    let query_result = sqlx::query_as!(
        GameModel,
    ...
    match query_result {
        Ok(game) => {
            let game_response = serde_json::json!({
                "status": "success", "data": serde_json::json!({
                    "game": game
                })});
            return HttpResponse::Ok().json(game_response);
        }
        Err(e) => {
            if e.to_string().contains("duplicate key value violates unique constraints") {
...
async fn update_game(path: web::Path<uuid::Uuid>, data: web::Data<AppState>, body: web::Json<UpdateGameSchema>) -> impl Responder {
    ...
    match query_result {
        Ok(game) => {
            let game_response = serde_json::json!({"status": "success", "data": serde_json::json!({ "game": game })});
            return HttpResponse::Ok().json(game_response);
        }
        Err(_) => {
            let message = format!("Note with ID: {} not found", game_id);
